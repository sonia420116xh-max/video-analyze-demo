...
历史分析详情页还提供“一键复制”脚本复刻能力。用户选择一条已完成拆解后，可以输入或上传自己的带货商品，系统会复用源视频的爆款公式、分镜节奏、卖点角度和黄金 3 秒钩子逻辑，生成新商品脚本。该功能通过 `POST /api/analyses/{analysis_id}/script-copy` 调用模型即时生成结果，不会把用户上传的产品图片保存到历史记录。

前端会通过 `POST /api/product-selling-points` 让模型根据商品标题、品类、补充内容和可选图片生成默认卖点；用户可以继续编辑。模型生成失败时，前端才使用本地规则兜底。点击生成脚本时，前端会比较用户商品推断类目和参考视频的 `product_classification`，不一致时会二次确认后再提交后端。
